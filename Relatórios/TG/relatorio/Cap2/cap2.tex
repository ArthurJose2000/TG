...
\centering
\begin{forest}
 [3 3,s sep=12mm,circle,draw,fill=red!100
 	[2 3,circle,draw
 		[2 2,circle,draw
 			[1 2,circle,draw
 				[1 3,circle,draw]]]]
...
Figure \ref{maze_example_graph_intervals} presents the convergence intervals of the cells (nodes) of a fictional maze when a given agent with an interval $a_{1}=[0,\frac{1}{4}[$ explores it based on Pseudocode \ref{pseudocode_1}. The letters in the tree indicate the visit order, from $a$ to $j$.

Thus, the goal of this work aims to establish an efficient algorithm that interrelates agent intervals to node intervals. Furthermore, the authors observed that a mixed radix representation to the agents' path is a powerful mathematical tool for relating the path to the interval of the node that the agent is visiting. Section \ref{section_models_mixed_radix} discusses the mixed radix representation.

\begin{figure}[ht!]
\centering
\begin{forest}

 [a,s sep=12mm,circle,draw,label=below:{$[0,1]$}
 	[b,s sep=12mm,circle,draw,label=below:{$[0,\frac{1}{2}[$}
 		[c,s sep=12mm,circle,draw,label=below:{$[0,\frac{1}{4}[$}
 			[d,circle,draw,label=below:{$[0,\frac{1}{8}[$}]
 			[e,circle,draw,label=below:{$[\frac{1}{8},\frac{1}{4}[$}]]
 		[f,circle,draw,label=below:{$[\frac{1}{4},\frac{1}{2}[$}]]
 	[g,circle,draw,label=right:{$[\frac{1}{2},1[$}
 		[h,s sep=12mm,circle,draw,label=below:{$[\frac{1}{2},1]$}
 		[i,circle,draw,label=below:{$[\frac{1}{2},\frac{3}{4}[$}]
 		[j,circle,draw,fill=green!100,label=below:{$[\frac{3}{4},1]$}]]]]
