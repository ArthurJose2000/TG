...
As pointed out in Section \ref{section_definitions_graph}, a graph may be represented as an adjacency list. Thus, considering an unknown maze as an adjacency list $A$, where the firstly explored node (root) $v_{1}$ has a convergence interval equal to $[0,1]$, while the other nodes $v_{i}$ of the maze are unknown, i.e., an agent don't know their convergence intervals before visit them, the Pseudocode \ref{pseudocode_1} establishes the convergence intervals of each node. As mentioned previously, there is no communication between agents, therefore each agent needs to explore the maze individually, and calculates by itself the convergence intervals of each node. Furthermore, it is important to emphasize that each agent considers individually the maze as tree, where the root is the agent's starting node.
...
\State $visited\_nodes \gets [$ $]$ \hspace{0.95cm} \slash\slash \hspace{0.1cm} visited nodes by the agent is initially empty

\State $intervals[v_{1}$] $\gets [0,1]$ \hspace{0.7cm} \slash\slash \hspace{0.1cm} $v_{1}$ is the root

\State $A [0][0] \gets 0$ \hspace{2.35cm} \slash\slash \hspace{0.1cm} adjacency list $A$ is initially $1 \times 1$

\State $A \gets complete(A, v_{1})$ \hspace{0.75cm} \slash\slash \hspace{0.1cm} check the children of the node $v_{1}$ and complete $A$\\

\\

\textbf{while} $current\_node$ is not the goal cell \textbf{do}\\

\hspace{0.3cm} \textbf{if} $current\_node$ has not been already visited \textbf{then}\\
...
\hspace{0.6cm} \textbf{foreach} children $v_{j}$ of $current\_node$ \textbf{do}\\
...
\hspace{0.6cm} CORRIGIR$current\_node \gets get\_parent(current\_node)$\\

\hspace{0.3cm} \textbf{end if}\\

\hspace{0.3cm} \textbf{foreach} children $v_{j}$ of $current\_node$ \textbf{do}\\

\hspace{0.6cm} \textbf{if} $v_{j}$ has been already visited \textbf{then}\\
...
\hspace{1.2cm} $current\_node \gets v_{i}$\\

\hspace{1.2cm} \textbf{break} \\

\hspace{0.9cm} \textbf{else}\\

\hspace{1.2cm} \textbf{continue} \\
...
, as presented in Figure \ref{maze_example_graph_intervals}, that represents the maze in Figure \ref{maze_example}.

Thus, the goal of this work aims to establish an efficient algorithm that interrelates the agent intervals to the graph intervals. Furthermore, a mixed radix representation to the agent path has been studied.
